...
| I31 | **Tdarr**: single generic `POST /api/v2/cruddb` endpoint over its LokiJS collections, no auth by default; `stats` maps `StatisticsJSONDB` (transcoded/health-check counts, space saved) plus a busy/total worker count flattened across `NodeJSONDB`; `staged` (list) keeps only `FileJSONDB` rows with `TranscodeDecisionMaker` `queued`/`error` and trims the path to a filename; missing/renamed fields fall back to 0/empty instead of throwing (unverified against a live instance — see README caveat) | ✅ `unit/integrations.tdarr` · `api/integrations` |
