...
| I33 | **Uptime Kuma** (`mergeGroup: 'monitor'`): public status-page JSON, no auth — `GET /api/status-page/{slug}` for the monitor list (`publicGroupList[].monitorList[]`), `GET /api/status-page/heartbeat/{slug}` for `heartbeatList` (per-monitor array, status 0=down/1=up/2=pending/3=maintenance — last entry wins) + `uptimeList` (`<id>_24` ratio); `status` maps the latest heartbeat to up/down/warn, a monitor with no heartbeat yet is `paused`, `detail` carries the 24h uptime %; `stats` counts up/down/total and averages the 24h uptime across monitors | ✅ `unit/integrations.uptimekuma` · `api/integrations` |
| I34 | **Gatus** (`mergeGroup: 'monitor'`): `GET /api/v1/endpoints/statuses`, no auth by default; `status` reads each endpoint's last `results[]` entry's `success` (up/down), an endpoint with an empty `results[]` is `paused`, `detail` carries the endpoint's `group`; `stats` counts healthy vs down from the same last-result check | ✅ `unit/integrations.gatus` · `api/integrations` |
| I35 | **Healthchecks** (`mergeGroup: 'monitor'`, works with self-hosted or hosted healthchecks.io): `GET /api/v3/checks/` with `X-Api-Key`; each check's `status` (new/up/grace/down/paused) maps to the shared state enum — `grace`→`warn`, `new`→`paused` alongside `paused` itself; `stats` counts by status | ✅ `unit/integrations.healthchecks` · `api/integrations` |
| I36 | **Grafana**: service-account token as `Authorization: Bearer`; `stats` sums `/api/search?type=dash-db` (dashboards) + `/api/datasources` (data sources) + firing-alert count; `alerts` (list) reads `/api/alertmanager/grafana/api/v2/alerts`, keeping only `status.state === 'active'`, with a summary/severity subtitle, or a single "No firing alerts" row when clean; the alerts call is best-effort (`.catch(() => [])`) so a disabled/misconfigured Alertmanager doesn't fail the whole poll | ✅ `unit/integrations.grafana` · `api/integrations` |
| I37 | **Speedtest Tracker**: `GET /api/v1/results/latest` for the current reading, `GET /api/v1/results` for a 24h average (filtered client-side by `created_at`); API token optional (many installs leave the API open on the LAN); falls back to the latest reading when the history call fails or returns nothing usable, so "Avg" never blanks out to 0 on a partial API | ✅ `unit/integrations.speedtest` · `api/integrations` |
| I38 | **Scrutiny**: `GET /api/summary`, no auth by default; response is `{data: {<wwn>: {device, smart}}}` keyed by drive WWN (not an array) — `Object.values` flattens it; `status` fails a drive when `smart.status` is non-zero, `detail` carries the temperature; falls back from `device_name` to `model_name` when the former is missing; `stats` counts passed vs failed | ✅ `unit/integrations.scrutiny` · `api/integrations` |
