...
| I32 | **What's Up Docker**: `GET /api/containers`, no auth by default; `list` shows only `updateAvailable` containers with an old→new tag subtitle, or a single "All containers up to date" row when none; `stats` counts monitored/updates-available/up-to-date | ✅ `unit/integrations.whatsupdocker` · `api/integrations` |
