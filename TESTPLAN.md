# selfdash — Test Plan & Case Catalog

The single source of truth for **what "working" means** in selfdash. Every feature,
route, and shared helper is listed here with its test cases and where they are
verified. Run this after **every** change; extend it whenever you add a feature.

- **How to run:** [`test/README.md`](test/README.md)
- **Automated suite:** `npm test` (unit + API) — 150+ cases, ~10s, no network, no browser
- **Full suite:** `npm run test:all` (adds the browser smoke; needs Chrome)

---

## 1. Validation gate for every change

Before a change is considered done:

1. `npm test` is green (`pretest` rebuilds `public/` first, so a broken frontend
   build fails here too).
2. If the change touched the frontend UI or `web/appearance.js`, also run
   `npm run test:e2e` (or do the manual smoke in §9) — a real browser render.
3. If the change **added or changed behaviour**, this file has a new or updated
   case for it **and** a matching automated test. A PR that changes behaviour
   without touching `test/` is incomplete.
4. `git grep -n 'TODO\|FIXME' test/` is not growing.

Coverage philosophy: **every HTTP route and every pure helper in `src/shared` and
`src/lib` has automated tests**; UI wiring that needs a DOM is covered by the
browser smoke plus the manual checklist in §9.

---

## 2. Test layers

| Layer | Dir | What it exercises | Speed | Deps |
|---|---|---|---|---|
| Unit | `test/unit/` | Pure logic: sanitizers, parsers, crypto, path mapping, DB migrations, i18n, integration registry | ~0.3s | none |
| API | `test/api/` | The real `src/server.js` booted as a child process against a temp SQLite DB; every route via `fetch` | ~8s | none (uses a local throwaway HTTP upstream for feed/customapi) |
| E2E smoke | `test/e2e/` | Headless Chrome loads the built app, hydrates, applies an uploaded background | ~10s | Chrome/Chromium (`$CHROME_BIN`); **skips cleanly if absent** |

Helpers: `test/helpers/server.mjs` (boot/stop a server, `request()` wrapper),
`test/helpers/tmpdir.mjs`, `test/helpers/fixtures.mjs` (sample RSS/Atom/ICS/JSON,
sample tiles & integration config).

---

## 3. Feature catalog

Legend: ✅ automated · 🖐️ manual (§9) · ⏭️ intentionally not covered (reason given)

### 3.1 Pages (`src/routes/pages.js`)
| # | Case | Where |
|---|---|---|
| P1 | Fresh DB seeds exactly one page `Home`/`home` | ✅ `unit/db.migrations` · `api/pages` |
| P2 | `GET /api/pages` returns pages ordered by `position, id` with parsed `options` | ✅ `api/pages` |
| P3 | `POST` creates a page, slugifies the name | ✅ `api/pages` |
| P4 | `POST` de-duplicates slugs (`media-center`, `media-center-2`) | ✅ `api/pages` |
| P5 | `POST` rejects blank/whitespace name (400) | ✅ `api/pages` |
| P6 | `PATCH` updates name + legacy `background` string | ✅ `api/pages` |
| P7 | `PATCH` sanitizes `options` (grid clamp, background clamp, unknown keys dropped; per-page `appearance` keeps a known `theme` + `#rrggbb` accent/textColor only; `customJs` stored, `customJsEnabled` only when truthy) | ✅ `api/pages` · `unit/shared.misc` |
| P8 | `PATCH` **merges** options rather than replacing | ✅ `api/pages` |
| P9 | `PATCH` unknown page → 404 | ✅ `api/pages` |
| P10 | `DELETE` a page (204); tiles cascade-delete | ✅ `api/pages` · `unit/db.migrations` |
| P11 | `DELETE` the last remaining page → 400 | ✅ `api/pages` |

### 3.2 Tiles (`src/routes/tiles.js`, `src/shared/tileConfig.js`)
| # | Case | Where |
|---|---|---|
| T1 | Create `link` tile; response has parsed `config: {}` and `open_mode: newtab` | ✅ `api/tiles` |
| T2 | `link` tile without a valid `http(s)` url → 400 | ✅ `api/tiles` · `unit/shared.tileConfig` |
| T3 | Create panel tile (`clock`) — sanitized config, `url: null` | ✅ `api/tiles` |
| T4 | `iframe` is its own tile type (not a link `open_mode`): requires a valid `http(s)` `config.url` (else 400), builds `{url,sizing,aspectRatio,height,sandbox}` in `config` with `url: null` on the tile itself; bad aspect → `16/9`; bad sandbox tokens dropped; a link tile's `open_mode` can no longer be set to `iframe` (falls back to `newtab`) | ✅ `api/tiles` · `unit/shared.tileConfig` |
| T5 | `widget` tile requires an existing `integration_id` (else 400) | ✅ `api/tiles` |
| T6 | Unknown `type` falls back to `link` | ✅ `api/tiles` · `unit/shared.tileConfig` |
| T7 | Geometry clamped on create (`w≤6,h≤6,x≤11,y≥0`) and on patch | ✅ `api/tiles` |
| T8 | `GET` tiles ordered by `position` | ✅ `api/tiles` |
| T9 | `PATCH` edits in place; switching type re-sanitizes config and clears `url` | ✅ `api/tiles` |
| T10 | `POST /reorder` sets positions from id order | ✅ `api/tiles` |
| T11 | `POST /reorder` non-array `order` → 400 | ✅ `api/tiles` |
| T12 | `DELETE` tile (204); repeat → 404 | ✅ `api/tiles` |
| T13 | `POST` to unknown page → 404 | ✅ `api/tiles` |
| T14 | Per-type config sanitizers (clock/weather/notes/search/rss/calendar/bookmarks/customapi/resources) coerce, clamp, drop unknown keys | ✅ `unit/shared.tileConfig` |
| T15 | `commonConfig` — group trim, appearance hex validation, `hideTitle` | ✅ `unit/shared.tileConfig` |
| T16 | 2-D packing: `occupancyOf` / `placeBox` / `nextFreeSlot` first-fit; `placeInGroup` packs only against same-group tiles and honours `exceptId` (the tile being moved) — used so adding a tile or changing a tile's group drops it in the target group's first free slot instead of overlapping | ✅ `unit/shared.gridPack` |
| T17 | Size presets ↔ `w×h` round-trip | ✅ `unit/shared.misc` |
| T18 | Tile drag/resize UI, group collapse (localStorage), "Add tile" slot picker | 🖐️ §9 |
| T18b | Group assignment is a radio picker (None / existing groups / "New group…"), not free text; edit-mode group headers rename (patches every member tile, collapse state migrates) and remove (members become ungrouped, none deleted) | 🖐️ §9.5b |
| T19 | `widgetConfig`: dedupes/caps `views` (max 8); `moreIntegrationIds` drops self-refs, dangling ids, non-integers, dedupes, caps at 12, and is dropped entirely unless exactly one view is selected (server-enforced, not just the tile modal's UI) | ✅ `unit/shared.tileConfig` · `api/tiles` |
| T20 | `mergeModels` ("Also include"): `queue`/`list` concatenate rows and tag each `subtitle` with its source (e.g. which download client); `calendar` merges + sorts by `ts` with a `source` tag; `status` concatenates monitors and tags each `detail` with its source (existing `detail` kept, source appended); `stats`/`nowplaying`/unknown don't merge (→ null); missing / wrong-type sources are skipped | ✅ `unit/shared.mergeModels` |
| T20b | **E1** — the `status` WidgetModel (`{label, state:'up'\|'down'\|'warn'\|'paused', detail?}`): `StatusView` in `WidgetTile.jsx` renders one colored-dot row per item (green/red/amber/grey for up/down/warn/paused); registered in `RENDERERS` alongside stats/nowplaying/queue/list/calendar; `status` is a scrollable view type (`SCROLLABLE_VIEW_TYPES` in `tileSizes.js`, same bucket as queue/list) so `autoLinkTileHeight` leaves its height alone | ✅ `unit/shared.mergeModels` · `unit/shared.misc` · 🖐️ (visual render, see §9) |
| T21 | "Also include" candidate list = integrations sharing the primary's `mergeGroup` **and** the picked view key — so a download-client queue merges qbittorrent + sabnzbd only, never radarr/sonarr | 🖐️ §9 (filter logic in `TileModal.jsx`; `mergeGroup` values covered by I1) |
| T22 | `commonConfig` keeps `appearance.textColor` only when a `#rrggbb` hex; `TileCard` maps it (and a derived `--text-dim`) to CSS vars on the tile root | ✅ `unit/shared.tileConfig` (apply path in `TileCard.jsx`) |
| T23 | `contrast.js`: `parseColor` (hex 3/6, rgb, rgba flattened over white), `contrastRatio` (21 for black/white, 1 for equal, order-independent, null on unparseable); drives the low-contrast hint on the text-colour pickers | ✅ `unit/shared.contrast` |
| T24 | `themes.js` `THEMES` list (8 built-ins incl. `dracula`, `oled`) is the single source for the settings route, page-options sanitiser, and settings UI | ✅ `api/settings` (accepts `dracula`/`oled`, rejects unknown) |
| T25 | `color.js` `isHexColor` (only `#rrggbb`, any case; rejects shorthand/names/rgb()/non-string) + `dimmedTextColor` — the one hex check reused by `tileConfig`, `pageOptions`, the settings route, and the appearance UI | ✅ `unit/shared.color` |
| T26 | `composePorts.js` `uniqueHostPorts` — de-dupe + numeric sort + protocol-normalise of published host ports, shared by the panel-wide strip and each stack's summary chips | ✅ `unit/shared.composePorts` |
| T27 | A `link` tile can optionally attach an integration ("Include integration data" in the tile modal), carrying the same `config.views`/`moreIntegrationIds` a widget tile does; `integration_id` is validated when given (400 on a dangling id) and clearable via `PATCH` | ✅ `api/tiles` |
| T28 | The `0003_iframe_own_type` migration promotes a pre-upgrade `link`+`open_mode:iframe` row to `type: iframe`, moving its `url` column into `config.url` and resetting `open_mode`; the YAML importer applies the same upgrade to an old-shaped exported doc on import | ✅ `unit/db.migrations` · `api/config` |
| T29 | `autoLinkTileHeight`: a combined link+integration tile's single non-scrollable view (`stats`/`nowplaying`/`calendar`) is bumped to the no-scroll minimum height (never shrunk below whatever was already picked); a scrollable view (`queue`/`list`) or an unknown type (integration not polled yet) leaves the height alone — wired in `TileModal.jsx`'s submit for a single selected (or default) view only, so a `queue`/`list` — a download queue, "recently imported" — keeps scrolling as intended | ✅ `unit/shared.misc` |
| T30 | `bookmarks` sanitizer: each link's `column` is clamped to 1-4 independent of the current `columns` count (so lowering columns and raising it back doesn't lose an assignment); missing/invalid values default to 1 | ✅ `unit/shared.tileConfig` |

### 3.3 Settings (`src/routes/settings.js`)
| # | Case | Where |
|---|---|---|
| S1 | `GET` returns full object = `DEFAULTS` ⊕ stored | ✅ `api/settings` |
| S2 | `PATCH` persists a valid subset, survives reload | ✅ `api/settings` |
| S3 | Enum/format validation: `theme`, `dark_mode`, `font_family`, `locale`, `accent` + `text_color` (#rrggbb), non-empty `site_title` | ✅ `api/settings` |
| S4 | Nullable fields cleared with `null` or `''` (`global_background`, `favicon`, `text_color`) | ✅ `api/settings` |
| S5 | `custom_css` / `custom_js` truncated to 40 000 chars, not rejected | ✅ `api/settings` |
| S6 | `compose_scan_dir` must be absolute | ✅ `api/settings` |
| S7 | `compose_scan_page_id` accepts a page id, `"all"`, `null` | 🖐️ (add ✅ when touched) |
| S8 | Appearance applied live: theme/mode/accent/font/title/favicon/**background** | ✅ `e2e/smoke` (background) · 🖐️ rest |

### 3.4 Appearance / background (`web/appearance.js`, `public/style.css`)
| # | Case | Where |
|---|---|---|
| A1 | Bundled `app.js` toggles `data-has-bg` on **`<body>`** (not `<html>`) | ✅ `api/health` (bundle assertion) |
| A2 | Uploaded/URL background renders full-page behind tiles | ✅ `e2e/smoke` · 🖐️ §9 |
| A3 | `--page-bg-blur/dim/opacity` vars honour per-page `options.background` | 🖐️ §9 |
| A4 | Per-page background overrides global; legacy string still works | 🖐️ §9 |
| A5 | Custom CSS (global + per-page) injected; opt-in custom JS executes | 🖐️ §9 |

### 3.5 Uploads (`src/routes/uploads.js`)
| # | Case | Where |
|---|---|---|
| U1 | PNG upload → `201 {url:/uploads/<uuid>.png}`, then that URL serves `image/png` | ✅ `api/uploads` |
| U2 | Each allowed mime → correct extension (png/jpg/gif/webp/svg) | ✅ `api/uploads` (png, svg) |
| U3 | Unsupported mime → 400 | ✅ `api/uploads` |
| U4 | No file part → 400 | ✅ `api/uploads` |
| U5 | > 5 MB → 413 | ⏭️ contract documented; skipped to keep the suite fast/quiet |

### 3.6 Integrations (`src/routes/integrations.js`, `src/integrations/*`)
| # | Case | Where |
|---|---|---|
| I1 | `GET /available` lists shipped integrations, each with `configSchema.fields` + `defaultInterval` + `views` catalog (`{viewKey: label}`, empty schema no longer carries a "Show" field) + `mergeGroup` (`download` for qbittorrent/sabnzbd, `arr` for radarr/sonarr/readarr/lidarr/bazarr, `requests` for overseerr/jellyseerr, `dns` for pihole/adguard, else the key) | ✅ `api/integrations` |
| I2 | Every `*.integration.js` registers under its static `key`, implements `fetchData()`, has a well-formed schema | ✅ `unit/integrations.registry` |
| I3 | Runtime-dropped `DATA_DIR/integrations/*.integration.js` overrides a shipped key | ✅ `unit/integrations.registry` |
| I4 | `POST` unknown key → 400 | ✅ `api/integrations` |
| I5 | `POST` missing required config → 400 (per `validateConfig`) | ✅ `api/integrations` · `unit/integrations.configCodec` |
| I6 | `POST` OK: interval floored at `POLL_MIN_INTERVAL`; response masks password fields to `true/false` | ✅ `api/integrations` |
| I7 | `GET` list returns masked config | ✅ `api/integrations` |
| I8 | `PATCH` rename/enable/interval without re-supplying config | ✅ `api/integrations` |
| I9 | `PATCH` blank password keeps the stored secret (`mergeConfig`) | ✅ `api/integrations` · `unit/integrations.configCodec` |
| I10 | `PATCH` validates the merged config → 400 | ✅ `api/integrations` |
| I11 | `POST /:id/poll` runs immediately, records `last_status`/`last_error` | ✅ `api/integrations` |
| I12 | `DELETE` (204); repeat → 404; unschedules poller; also prunes the id out of any tile's `config.moreIntegrationIds` (not FK-tracked, since it lives in JSON) | ✅ `api/integrations` |
| I13 | Config at rest: AES-256-GCM round-trip when `APP_SECRET` set; plaintext JSON when not | ✅ `unit/lib.crypto` · `unit/integrations.configCodec` |
| I14 | Wrong key / tampered ciphertext fails closed | ✅ `unit/lib.crypto` |
| I15 | Per-integration upstream parsing (qbit torrents, tautulli now-playing, …) | ⏭️ needs live services; covered indirectly by I2 + view helpers |
| I16 | Sonarr/Radarr `calendar` view: maps upstream records to `{ts,title,subtitle,image}`, drops undated records, sorts ascending, picks the earliest of several release dates, carries the series/movie poster URL through as `image`, honors `config.upcomingDays` for the window | ✅ `unit/integrations.arrCalendar` |
| I17 | `runAllViews`: fetches every declared view every poll (not just a configured subset), keyed by view key in `byView`; one view failing falls back to that view's last-good data tagged `stale: true` (via `ctx.previous`, wired by `poller/scheduler.js` from `last_data_json`) instead of blanking it out, or `{type:'error'}` if there's no previous data to fall back to; every view failing throws so the poll is marked unreachable and last-good data stays on screen | ✅ `unit/integrations.views` |
| I18 | Merged download-queue tiles (qbittorrent + sabnzbd, "Also include"): a transient per-view failure on one source no longer drops that source's rows from the merged queue for the cycle — `WidgetTile.jsx` shows a "stale data" banner naming the affected source(s) instead | 🖐️ (see §9) |
| I18c | **Phase 5 enabler**: `HttpClient.fetch()`/`fetchJson()` accept `insecureTLS: true` (an `allowInsecureTLS` checkbox field surfaced by Proxmox/TrueNAS/Home Assistant/Nextcloud/UniFi) to skip TLS cert verification for appliances that ship a self-signed cert by default. Uses undici's own `fetch` + `Agent` together (not Node's global fetch) — pairing Node's built-in fetch with an externally-constructed `undici` Agent throws (`invalid onRequestStart method`) on a version mismatch between Node's internal vendored undici and the npm package. Proven against a real self-signed TLS handshake: rejects by default, accepts with the flag set | ✅ `unit/lib.httpClient` (real `https` server + a committed test-fixture cert, not a mock) |
| I19 | **Jellyfin / Emby** (`_embyBase.js`, confirmed live against both `jellyfin/jellyfin:10.10.7` and `emby/embyserver` — bare `/Sessions`/`/Items/Counts` paths work unprefixed on both): `nowplaying` view drops idle sessions (no `NowPlayingItem`), maps title/subtitle (series → album → joined artists → user), clamps `progress` into 0–1, tolerates a missing `PlayState` or a non-array `/Sessions` body, and builds the image URL from the parent `SeriesId`/`AlbumId` with the token as an `api_key` query param; `stats` view maps `/Items/Counts` with missing counts defaulting to 0; both send `X-Emby-Token`; the two integrations expose an identical view catalog | ✅ `unit/integrations.jellyfin` · `api/integrations` |
| I20 | **Lidarr** (`_arrBase.js`, v1 API, confirmed live against `linuxserver/lidarr` with a real added/monitored artist+album): `queue` → "Artist — Album" title + status + `progressFromSize`; `stats` → artist/monitored/track counts with `Missing = max(0, Σ trackCount − Σ trackFileCount)`; `calendar` buckets by `releaseDate`, drops undated albums, sorts ascending, carries the artist poster; `upcoming` (list) keeps undated rows with no date subtitle and sorts them last; `history` → "Artist — Album" + quality/relative-time subtitle; all calls hit `/api/v1/` (never `/api/v3/`); `config.upcomingDays` widens the calendar window | ✅ `unit/integrations.lidarr` · `api/integrations` |
| I21 | **Bazarr** (own `/api` + `X-API-KEY`, confirmed live against `linuxserver/bazarr`): `stats` maps `/api/badges` (episodes/movies/health/providers, missing keys → 0); `wanted` merges episode + movie rows with a missing-languages subtitle and obeys `listLimit` (max 50); `history` merges episode + movie downloads with a `language · provider · time` subtitle; `health` maps `issue`/`object` and emits a single "No health issues" row when clean; `mergeGroup: 'arr'` | ✅ `unit/integrations.bazarr` · `api/integrations` |
| I22 | **Jellyseerr** shares `_seerrBase.js` with Overseerr (confirmed live against `fallenbagel/jellyseerr` set up through the real Jellyfin-connect wizard — requires `serverType: 2` in the `/api/v1/auth/jellyfin` body or it 500s with `NO_ADMIN_USER` even with valid admin creds — plus a real TMDB-backed movie request): identical view catalog + config schema, `mergeGroup: 'requests'` on both; the extracted base still resolves list-view titles/posters through the server's TMDB proxy (w154), tags 4K, and maps the request/issue/media count + status views; sends `X-Api-Key` | ✅ `unit/integrations.jellyseerr` · `api/integrations` |
| I23 | **Pi-hole** (`mergeGroup: 'dns'`, confirmed live against `pihole/pihole` v6 with real (un)blocked DNS queries): tries the v6 API first — `POST /api/auth` → session `sid`, `GET /api/stats/summary` mapped to Queries/Blocked/Blocked %/Domains/Clients, `X-FTL-SID` header on the summary call, best-effort `DELETE /api/auth` to free the session; a passwordless box (`sid` null, `valid` true) still reads the summary with no SID header and no delete; falls back to the v5 `GET /admin/api.php?summaryRaw&auth=<token>` when v6 `/api/auth` is absent; both APIs failing rejects the poll with a combined `v6: … | v5: …` message | ✅ `unit/integrations.pihole` · `api/integrations` |
| I24 | **AdGuard Home** (`mergeGroup: 'dns'`, confirmed live against `adguard/adguardhome` — DNS-level ad blocking confirmed via a real `dig` query against its resolver — with real blocked-domain data): `stats` maps `/control/stats` (Queries, Blocked, computed Blocked %, `avg_processing_time`×1000 as ms) + `/control/status` protection flag (`"off"` only when `protection_enabled === false`; a failed `/status` defaults to `"on"`); `blocked` view flattens the `[{domain: count}]` objects into a list; HTTP basic-auth header sent only when a username is configured | ✅ `unit/integrations.adguard` · `api/integrations` |
| I25 | **Portainer** (confirmed live against `portainer/portainer-ce:2.45.0` with the real local Docker socket): `stats` sums the `/api/endpoints` snapshot counts (Running/Stopped/Images/Volumes/Stacks); `down` lists stopped + unhealthy containers from `Snapshots[0].DockerSnapshotRaw.Containers`, skipping healthy running ones, prefixing the environment name when >1 endpoint, and falling back to the Docker proxy `/api/endpoints/{id}/docker/containers/json?all=1` when the raw list is absent — on 2.45.0 the snapshot never carries `Containers` at all, so the fallback is the path exercised in practice, not a rare edge case; `endpointId` narrows to one environment; `X-API-Key` header | ✅ `unit/integrations.portainer` · `api/integrations` |
| I26 | **Traefik** (confirmed live against `traefik:latest` with `--api.insecure`): `stats` maps `/api/overview` totals (HTTP routers/services/middlewares, TCP routers) and sums the per-kind `errors`; a bare `{}` overview yields all-zeros; basic-auth header only when a username is set | ✅ `unit/integrations.traefik` · `api/integrations` |
| I27 | **Nginx Proxy Manager** (`npm`, confirmed live against `jc21/nginx-proxy-manager` 2.15.1 — that version has no seeded `admin@example.com`/`changeme` account; the first admin is bootstrapped via an unauthenticated `POST /api/users` when zero users exist yet, a real behavior change from older NPM releases, but the `/api/tokens` login contract our integration depends on is unchanged): `POST /api/tokens` bearer token cached per (url,email) — fetched once and reused across polls, concurrent first-poll logins deduped via an in-flight promise, one transparent re-auth on a 401; `stats` counts proxy/redirection/stream hosts plus disabled + offline proxy hosts; `certs` lists certificates expiring within `certDays` (default 30), soonest-first, with a `(Nd)` / `(expired)` subtitle | ✅ `unit/integrations.npm` · `api/integrations` |
| I28 | **Transmission** (`mergeGroup: 'download'`, confirmed live against `linuxserver/transmission` with a real (paused) magnet torrent — status codes 0/4 observed exactly as mapped): RPC `POST /transmission/rpc`; a request without a valid `X-Transmission-Session-Id` gets a 409 carrying the current one in a response header, cached per RPC URL and retried once; `queue` maps numeric status codes to a label and passes `percentDone` through as `progress`; `stats` counts by status and sums `rateDownload`/`rateUpload` through `fmtRate`; basic-auth header only when a username is configured | ✅ `unit/integrations.transmission` · `api/integrations` |
| I29 | **Deluge** (`mergeGroup: 'download'`, confirmed live against `linuxserver/deluge` — see README caveat): JSON-RPC `/json`; `auth.login` sets a `_session_id` cookie cached per url (concurrent first-poll logins deduped), any RPC error drops the cached cookie and retries the login once (no HTTP status to branch on — Deluge reports auth failure inside a 200 body); a disconnected web UI (`web.update_ui` returning `connected: false, torrents: null` — the process starts out disconnected from its daemon until something calls `web.connect`) is detected and self-healed by connecting to the first `web.get_hosts` entry, deduped via an in-flight promise, then retried once; `queue` converts Deluge's 0-100 `progress` to the model's 0-1; `stats` counts Downloading/Seeding and sums payload rates | ✅ `unit/integrations.deluge` · `api/integrations` |
| I30 | **NZBGet** (`mergeGroup: 'download'`, confirmed live against `linuxserver/nzbget`): JSON-RPC `/jsonrpc` with HTTP basic auth (control username/password, required by default); `queue` computes `progress` from `1 - RemainingSizeMB/FileSizeMB` per group; `stats` maps `status` (`RemainingSizeMB`, `DownloadRate` via `fmtRate`, `DownloadPaused`) plus the `listgroups` count | ✅ `unit/integrations.nzbget` · `api/integrations` |
| I31 | **Tdarr** (confirmed live against `haveagitgat/tdarr` — see README caveat): `POST /api/v2/cruddb` (`mode: 'getAll'`, not `'find'` — that 400s) for `StatisticsJSONDB`/`FileJSONDB`, plus `GET /api/v2/get-nodes` for live worker state (`NodeJSONDB` via cruddb carries config only, no `workers`); `stats` maps `table0Count`/`table2Count` as Queued/Errored (the server's own queue-tab counts), `totalTranscodeCount`, `sizeDiff`, and busy/capacity summed from each node's `workers` object + `workerLimits`; `staged` (list) keeps only `FileJSONDB` rows with `TranscodeDecisionMaker` `'Queued'`/`'Transcode error'` (Title Case) and trims the path to a filename; missing/renamed fields fall back to 0/empty instead of throwing | ✅ `unit/integrations.tdarr` · `api/integrations` |
| I32 | **What's Up Docker** (confirmed live against `getwud/wud` watching the real local Docker socket, with an intentionally outdated `nginx:1.20` container as an update-available fixture): `GET /api/containers`, no auth by default; `list` shows only `updateAvailable` containers with an old→new tag subtitle, or a single "All containers up to date" row when none; `stats` counts monitored/updates-available/up-to-date | ✅ `unit/integrations.whatsupdocker` · `api/integrations` |
| I33 | **Uptime Kuma** (`mergeGroup: 'monitor'`, confirmed live against `louislam/uptime-kuma:1` — admin/monitor/status-page set up over its Socket.IO admin API, `saveStatusPage`'s real signature is `(slug, config, imgDataUrl, publicGroupList, callback)` with `config.domainNameList` required, easy to miss from the client alone): public status-page JSON, no auth — `GET /api/status-page/{slug}` for the monitor list (`publicGroupList[].monitorList[]`), `GET /api/status-page/heartbeat/{slug}` for `heartbeatList` (per-monitor array, status 0=down/1=up/2=pending/3=maintenance — last entry wins) + `uptimeList` (`<id>_24` ratio); `status` maps the latest heartbeat to up/down/warn, a monitor with no heartbeat yet is `paused`, `detail` carries the 24h uptime %; `stats` counts up/down/total and averages the 24h uptime across monitors; both a real up and a real down monitor confirmed | ✅ `unit/integrations.uptimekuma` · `api/integrations` |
| I34 | **Gatus** (`mergeGroup: 'monitor'`, confirmed live against `twinproduction/gatus` with a real passing + a real failing endpoint): `GET /api/v1/endpoints/statuses`, no auth by default; `status` reads each endpoint's last `results[]` entry's `success` (up/down), an endpoint with an empty `results[]` is `paused`, `detail` carries the endpoint's `group`; `stats` counts healthy vs down from the same last-result check | ✅ `unit/integrations.gatus` · `api/integrations` |
| I35 | **Healthchecks** (`mergeGroup: 'monitor'`, confirmed live against `healthchecks/healthchecks` — API keys live on `Project.api_key`, not `Profile`, in this version; up/down/paused/new all confirmed via `/ping/<uuid>`, `/ping/<uuid>/fail`, and the pause endpoint): `GET /api/v3/checks/` with `X-Api-Key`; each check's `status` (new/up/grace/down/paused) maps to the shared state enum — `grace`→`warn`, `new`→`paused` alongside `paused` itself; `stats` counts by status | ✅ `unit/integrations.healthchecks` · `api/integrations` |
| I36 | **Grafana** (confirmed live against `grafana/grafana` with a real dashboard, data source, and a real firing alert rule via the provisioning API): service-account token as `Authorization: Bearer`; `stats` sums `/api/search?type=dash-db` (dashboards) + `/api/datasources` (data sources) + firing-alert count; `alerts` (list) reads `/api/alertmanager/grafana/api/v2/alerts`, keeping only `status.state === 'active'`, with a summary/severity subtitle, or a single "No firing alerts" row when clean; the alerts call is best-effort (`.catch(() => [])`) so a disabled/misconfigured Alertmanager doesn't fail the whole poll | ✅ `unit/integrations.grafana` · `api/integrations` |
| I37 | **Speedtest Tracker** (confirmed live against `ghcr.io/alexjustesen/speedtest-tracker` with a real Ookla speedtest run — found and fixed two real bugs: the actual endpoint is `GET /api/speedtest/latest`, not `/api/v1/results/latest`, and there is no history/list endpoint at all, so the "24h average" feature was dropped entirely rather than kept as a fallback that always just repeats the latest reading; `download`/`upload` in the API response are already Mbps, converted server-side from the raw Ookla CLI's bytes/sec `bandwidth` field): `stats` reports the latest download/upload/ping/server name; the endpoint is public by default, an API token is optional and sent as a bearer header only when configured | ✅ `unit/integrations.speedtest` · `api/integrations` |
| I38 | **Scrutiny** (confirmed live against `ghcr.io/analogj/scrutiny:master-omnibus` — found and fixed a real bug: the per-device map is nested at `data.summary`, not `data` itself, so `Object.values(data.data)` was returning one bogus element (the whole summary map) instead of the individual devices; confirmed against both a real empty summary and two directly-seeded SQLite rows, one healthy one failing): `GET /api/summary`, no auth by default; response is `{success, data: {summary: {<wwn>: {device, smart?}}}}`; `status` fails a drive when `device.device_status` is non-zero (there's no `smart.status` field at all — `smart` only carries `temp`/`collector_date`/etc. and is absent until a device has a recent collector run), `detail` carries `smart.temp` when present; falls back from `device_name` to `model_name` when the former is missing; `stats` counts passed vs failed | ✅ `unit/integrations.scrutiny` · `api/integrations` |
| I39 | **Home Assistant** (confirmed live against `homeassistant/home-assistant:stable`, onboarded via `/api/onboarding/users` + `/auth/token` to get a real bearer token): `GET /api/states` with `Authorization: Bearer`; `stats` counts total entities + `automation.*` entities and appends one stat row per user-chosen entity ID (`config.entities`, comma-separated) — an unknown ID shows "unavailable" rather than throwing; `entities` (list) shows the chosen entities, or the first 20 states when none are chosen | ✅ `unit/integrations.homeassistant` · `api/integrations` |
| I40 | **Nextcloud** (confirmed live against `nextcloud:latest` with real serverinfo data): `GET /ocs/v2.php/apps/serverinfo/api/v1/info?format=json` with `OCS-APIRequest: true` + HTTP basic auth; `stats` maps `activeUsers.last24hours`, `storage.num_users`/`num_files`, `system.freespace` (via shared `fmtBytes`), and `system.apps.num_updates_available` — the last one is genuinely absent on a fresh install (nothing needs updating), so it falls back to 0 rather than crashing | ✅ `unit/integrations.nextcloud` · `api/integrations` |
| I41 | **Proxmox VE**: `GET /api2/json/cluster/resources` with `Authorization: PVEAPIToken=<tokenId>=<secret>`; one flat resource list told apart by `type` (node/qemu/lxc/storage); `stats` sums node CPU/RAM and storage usage into percentages, counts nodes online and VMs/LXCs running; `nodes` (list) is one row per node with status + CPU/RAM. **Not live-testable in this environment** — no Docker image exists for the actual Proxmox VE hypervisor (it's a full OS, not a container workload); built from Proxmox's documented, stable `cluster/resources` API and covered by mock-HTTP unit tests only | ✅ `unit/integrations.proxmox` · `api/integrations` |
| I42 | **TrueNAS**: `GET /api/v2.0/pool` + `/api/v2.0/alert/list` + `/api/v2.0/update/check_available`, `Authorization: Bearer <apiKey>`; `stats` counts pools/healthy/alerts and reports update availability; `pools` (list) is name + status, falling back to `ONLINE`/`unknown` when a pool's `status` field is absent. **Not live-testable in this environment** — TrueNAS CORE/SCALE is a full NAS appliance OS with no Docker image; built from TrueNAS's documented REST v2.0 API and covered by mock-HTTP unit tests only | ✅ `unit/integrations.truenas` · `api/integrations` |
| I43 | **UniFi Network** (confirmed live against `linuxserver/unifi-network-application` + a real self-signed cert, onboarded via its actual setup-wizard call `POST /api/cmd/sitemgr {cmd:"add-default-admin",...}` reverse-engineered from the shipped setup-wizard JS bundle): API-key auth (`X-API-Key`) or classic cookie login (`POST /api/login`, session deduped via an in-flight promise, one retry on a 401 for cookie auth only); `GET /api/s/{site}/stat/sta` (clients) + `/stat/device` (APs/gateway); `stats` splits wired vs WiFi clients via `is_wired`, counts APs (`type: 'uap'`) up via `state === 1`, and reads WAN from a `ugw`/`udm` gateway device's `state` — `unknown` when no gateway is present; `allowInsecureTLS` verified against the controller's real default self-signed cert (also exercises the shared `insecureTLS` enabler end-to-end, not just the isolated `unit/lib.httpClient` test) | ✅ `unit/integrations.unifi` · `api/integrations` |
| I44 | **Navidrome** (Phase 6, confirmed live against `deluan/navidrome` 0.63.2 — salted-token auth + `/rest/getScanStatus` `scanStatus` shape `{scanning, count, folderCount, lastScan, ...}` verified; `getNowPlaying` mapping not exercised with a live stream, no seeded audio): Subsonic API with salted-token auth — `t = md5(password + salt)` sent with `u`/`s`/`v=1.16.1`/`c=selfdash`/`f=json` on every call, the raw password never on the wire; `nowplaying` maps `/rest/getNowPlaying` `nowPlaying.entry` (coerced to an array when the server returns a lone object) to title + "artist · user"; `stats` reads `count` songs / `folderCount` when present / `scanning` flag; a `subsonic-response.status === "failed"` surfaces the upstream `error.message` | ✅ `unit/integrations.navidrome` · `api/integrations` |
| I45 | **Paperless-ngx** (Phase 6, confirmed live against `ghcr.io/paperless-ngx/paperless-ngx` with a real OCR'd document — all five stat fields + the null-inbox-tag fallback verified; on a fresh instance `documents_inbox`, `inbox_tag` and `inbox_tags` are all `null`): `Authorization: Token <token>`; `stats` maps `/api/statistics/` (`documents_total`, `documents_inbox`, `tag_count`, `correspondent_count`, `document_type_count`); `inbox` reads the inbox tag id(s) from the same payload — `inbox_tags` array on 2.x, single `inbox_tag` on 1.x — then `GET /api/documents/?tags__id__in=<ids>&ordering=-created&page_size=20`, with "Inbox is empty" / "No inbox tag configured" placeholder rows | ✅ `unit/integrations.paperlessngx` · `api/integrations` |
| I46 | **Komga** (Phase 6, confirmed live against `gotson/komga` with a seeded 1-series / 2-book library, one book marked read → 50%): HTTP basic auth; each `/api/v1/{series,books}` list endpoint returns a Spring `Page`, so `?size=1` reads `totalElements` cheaply; `stats` reports Series, Books, and Read % (`books read_status=READ` total ÷ books total, 0% not NaN when empty); the `size` param is appended with `?`/`&` depending on whether the path already carries a query | ✅ `unit/integrations.komga` · `api/integrations` |
| I47 | **Kavita** (Phase 6, confirmed live against `jvmilazz0/kavita` — the `POST /api/Plugin/authenticate?apiKey=&pluginName=selfdash` → JWT flow and the `/api/Stats/server/stats` field shape `{seriesCount, volumeCount, chapterCount, totalFiles, ...}` verified; `/api/Stats/user/read` returns an empty 200 body until the user has reading progress, which the per-call `.catch(() => ({}))` turns into "Words read = 0"): the JWT is cached per (url,key) with in-flight dedup, then sent as `Authorization: Bearer`; it is resolved up front so an auth failure fails the poll rather than rendering all-zeros; `stats` maps server stats (series/volumes/chapters — admin-only, best-effort) + user read (words/pages) | ✅ `unit/integrations.kavita` · `api/integrations` |
| I48 | **Miniflux** (Phase 6, confirmed live against `miniflux/miniflux` + Postgres — 30 unread / 2 feeds / 1 starred verified with a real `POST /v1/api-keys` token): `X-Auth-Token` header; `stats` sums `/v1/feeds/counters` `unreads` map for total unread, counts `/v1/feeds`, and reads `/v1/entries?starred=true&limit=1` `total` for starred — the feeds/starred calls are best-effort so a hiccup degrades them to 0 while unread still reports | ✅ `unit/integrations.miniflux` · `api/integrations` |
| I49 | **FreshRSS** (Phase 6, confirmed live against `freshrss/freshrss` with the Google Reader API enabled (`api_enabled` in `data/config.php`, not per-user) — ClientLogin handshake + 10 unread / 1 subscription verified; note the `Auth=<token>` value itself contains a `/` and is passed through verbatim): two-step auth — `POST /api/greader.php/accounts/ClientLogin` (form body) returns a plain-text `Auth=` line (parsed via `res.text()`), cached per (url,user), then `Authorization: GoogleLogin auth=<token>` on every call, one re-auth on a 401; `stats` reads overall unread from the `user/-/state/com.google/reading-list` row of `/reader/api/0/unread-count?output=json` (falling back to summing `feed/*` counts), and counts `/reader/api/0/subscription/list` | ✅ `unit/integrations.freshrss` · `api/integrations` |
| I50 | **Gotify** (Phase 6, confirmed live against `gotify/server` 3.1 with real pushed messages — Gotify fills an omitted per-message `title` with the sending application's name, so the `|| 'Message'` fallback is only a last resort): client token via `X-Gotify-Key`; `messages` (list) maps `/message?limit=20` to title + whitespace-collapsed body/date subtitle with a "No messages" placeholder; `stats` counts `/application` + `/client`, each best-effort | ✅ `unit/integrations.gotify` · `api/integrations` |
| I51 | **ntfy** (Phase 6, confirmed live against `binwiederhier/ntfy` with real published messages): `GET /<topic>/json?poll=1&since=<window>` returns NDJSON — parsed line-by-line via `res.text()`, keeping only `event === "message"`, sorted newest-first, capped at 20; the topic is URL-encoded, the lookback window defaults to `12h` (overridable), and a bearer token is sent only when configured; a non-OK HTTP status rejects | ✅ `unit/integrations.ntfy` · `api/integrations` |
| I52 | **Vikunja** (Phase 6, confirmed live against `vikunja/vikunja` v2.6 with real tasks + due dates, tested with both a `/login` JWT and a scoped `tk_` API token): `Authorization: Bearer <token>`; **the list endpoint is `GET /api/v1/tasks`, not `/tasks/all`** — the latter 400s "Invalid model provided" on current Vikunja (the fix found during live testing); query `?sort_by=due_date&order_by=asc&per_page=100`; a due date of `0001-01-01T00:00:00Z` counts as unset; `due` (list) shows open tasks with a real due date, prefixing "overdue — " vs "due "; `stats` counts open tasks, overdue, and due-today (local end-of-day). An API token must carry the `tasks: read_all` scope | ✅ `unit/integrations.vikunja` · `api/integrations` |
| I53 | **Frigate** (Phase 6, confirmed live against `ghcr.io/blakeblackshear/frigate:stable` 0.17.2 with no cameras configured — `/api/stats` `cameras` (`{}`), `service.storage['/media/frigate/recordings']` (`{total, used, free, mount_type}`), and `service.version` shapes verified, plus empty `/api/events` + `/api/events/summary`): no auth by default; `stats` counts `cameras` keys, sums `/api/events/summary` `count` for today's local `YYYY-MM-DD` rows (best-effort), and computes storage % from the recordings mount (or the first mount with a `total`), plus the version; `events` (list) maps `/api/events?limit=20` to "label · camera" + local start time, with placeholder rows when empty | ✅ `unit/integrations.frigate` · `api/integrations` |
| I54 | **Prometheus** (Phase 6, confirmed live against `prom/prometheus` scraping itself plus a deliberately-unreachable target — up/down counts + the down-target list carrying the real scrape error verified): optional HTTP basic auth; `stats` counts `/api/v1/targets` `activeTargets` by `health === 'up'` and `/api/v1/alerts` by `state === 'firing'` (not `pending`), the alerts call best-effort so a Prometheus without alerting still polls; `down` (list) shows unhealthy targets with `lastError`/instance subtitle or an "All targets up" row; a `status: "error"` body rejects | ✅ `unit/integrations.prometheus` · `api/integrations` |
| I55 | **Mastodon** (Phase 6, verified against the live `mastodon.social` `/api/v1/instance` response — Mastodon 4.8, the deprecated v1 `stats: {user_count, status_count, domain_count}` block is still served): `stats` reads that block (Users/Statuses/Connections) + version; falls back to `/api/v2/instance` (`usage.users.active_month` + version) when v1 is absent; both endpoints failing rejects the poll | ✅ `unit/integrations.mastodon` · `api/integrations` |
| I56 | **RomM** (Phase 6, confirmed live against `rommapp/romm` + MariaDB — `GET /api/stats` returns `{PLATFORMS, ROMS, SAVES, STATES, SCREENSHOTS, TOTAL_FILESIZE_BYTES}` exactly, publicly readable without auth on this version): `stats` maps Platforms/ROMs/Saves/States + `TOTAL_FILESIZE_BYTES` through the shared `fmtBytes`; HTTP basic auth sent only when a username is configured; missing fields default to 0 | ✅ `unit/integrations.romm` · `api/integrations` |

### 3.7 Tile data proxies (`src/routes/tileData.js`)
| # | Case | Where |
|---|---|---|
| D1 | `GET /api/tile/weather` requires `lat`+`lon` (400) | ✅ `api/tileData` |
| D2 | `GET /api/tile/weather` happy path (open-meteo) | ⏭️ live network; contract asserted via D1 + `unit/lib.apiMap` |
| D3 | `GET /api/tile/feed` rejects non-`http(s)` URL (400) | ✅ `api/tileData` |
| D4 | `GET /api/tile/feed` parses RSS (and ICS) from an upstream | ✅ `api/tileData` (local upstream) · `unit/lib.feedParse` |
| D5 | `GET /api/tile/feed` upstream error → 502 | ✅ `api/tileData` |
| D6 | `GET /api/tile/customapi/:id` → 404 if tile isn't a `customapi` | ✅ `api/tileData` |
| D7 | `customapi` maps JSON to a stats/list model (`buildModel` + `getPath`) | ✅ `api/tileData` · `unit/lib.apiMap` |
| D8 | `customapi` non-JSON body / upstream 5xx → 502 | ✅ `api/tileData` |
| D9 | `GET /api/host/stats` returns cpu/mem/disk (200) or a clean 500 in a locked-down sandbox; payload carries `netBusiest` (auto-pick hint for the config UI) | ✅ `api/tileData` |
| D9b | `fmtRate` uses adaptive units (B/s → KB/s → MB/s) so sub-MB/s throughput isn't flattened to "0.0 MB/s" | ✅ `unit/shared.misc` |
| D10 | `getPath` supports `a.b`, `a[0].b`, `a.b[].c` wildcard | ✅ `unit/lib.apiMap` |
| D11 | Feed/ICS parser edge cases: Atom alt links, CDATA/entities, ICS unfolding, all-day, horizon filter | ✅ `unit/lib.feedParse` |
| D12 | `parseFeed` item thumbnail: tries `media:thumbnail` → `media:content[medium=image]` → an image-typed `enclosure` → `itunes:image` → falls back to the first `<img>` inside the item's `content`/`description` HTML when no structured image tag exists (e.g. The Verge's Atom feed); no image anywhere → `image: ''`, not a throw | ✅ `unit/lib.feedParse` |

### 3.8 Tile health (`src/routes/health.js`)
| # | Case | Where |
|---|---|---|
| H1 | Reachable host (even `401/403/500`) → `online` with `code` | ✅ `api/tileData` (health check suite) |
| H2 | Dead port / bad host → `offline` | ✅ `api/tileData` |
| H3 | Non-`http(s)` entries filtered out | ✅ `api/tileData` |
| H4 | Empty `urls` → `{}` | ✅ `api/tileData` |
| H5 | 15 s response cache | 🖐️ (add ✅ when touched) |

### 3.9 Compose scan (`src/routes/composeScan.js`, `src/lib/composeScan.js`)
| # | Case | Where |
|---|---|---|
| C1 | Disabled by default → `{enabled:false,result:null}` | ✅ `api/composeScan` |
| C2 | Enabled, no dir → `result.error:"no directory configured"` | ✅ `api/composeScan` · `unit/lib.composeScan` |
| C3 | Enabled + dir → stacks with services, images, ports, volumes | ✅ `api/composeScan` · `unit/lib.composeScan` |
| C4 | Bad YAML captured in `errors[]`, never thrown | ✅ `unit/lib.composeScan` |
| C5 | `${VAR}` interpolation from sibling `.env` | ✅ `unit/lib.composeScan` |
| C6 | Settings change invalidates the cache | ✅ `api/composeScan` |
| C7 | Nonexistent / non-dir path reports a read error | ✅ `unit/lib.composeScan` |
| C8 | Walk bounded by depth / file count / file size; skips `node_modules`, `.git`, symlink escapes | ⏭️ limits in code; add ✅ if changed |
| C9 | `hostPortConflicts`: flags a `host/proto` published by 2+ services (naming each owner); tcp+udp on one number don't clash; a service repeating a mapping isn't a self-conflict; expose-only (no host) ports ignored | ✅ `unit/shared.composePorts` |
| C10 | Panel filter box narrows stacks by name/service/image/port substring; Expand-all / Collapse-all; per-page open state in `localStorage` (`selfdash:compose-open:<pageId>`) | 🖐️ §9.11 |

### 3.10 Backup (`src/routes/backup.js`)
| # | Case | Where |
|---|---|---|
| B1 | `GET /api/backup/export` streams a `.zip` with `selfdash.db` + `manifest.json` (v1) | ✅ `api/backup` |
| B2 | `POST /api/backup/import` rejects a non-`.zip` (400) | ✅ `api/backup` |
| B3 | `POST /api/backup/import` rejects a zip missing `selfdash.db` (400) | ✅ `api/backup` |
| B4 | `POST /api/backup/import` of a real export → `200 {ok:true}` then `process.exit(0)` (Docker restarts) | ✅ `api/backup` |
| B5 | Path-traversal entries in the archive rejected | ⏭️ guarded in code; add ✅ if changed |
| B6 | Pre-import snapshot written to `.pre-import-backup-*` | 🖐️ |

### 3.11 Config as code (`src/routes/config.js`, `src/lib/configFile.js`)
| # | Case | Where |
|---|---|---|
| G1 | `GET /api/config/export` → YAML v1 with settings/pages/tiles/integrations | ✅ `api/config` |
| G2 | Password fields exported as `${SELFDASH_SECRET_*}` placeholders; real secret never present | ✅ `api/config` |
| G3 | `POST /api/config/import` (raw YAML body) replaces everything transactionally | ✅ `api/config` |
| G4 | Import round-trip preserves page names + settings | ✅ `api/config` |
| G5 | Import rejects empty body / wrong `version` (400) | ✅ `api/config` |
| G6 | `?settings=0` leaves settings untouched | ✅ `api/config` |
| G7 | `${ENV}` resolution + `unresolvedSecrets` reporting on import | 🖐️ (add ✅ when touched) |
| G8 | Multipart `.yaml` upload path (vs raw body) | 🖐️ |
| G9 | A widget tile's `config.moreIntegrationIds` exports as portable integration `ref` strings (not raw db ids) and resolves back to the *new*, re-imported integration's id on import | ✅ `api/config` |
| G10 | A link tile's optional attached integration exports/imports the same way a widget tile's does (portable ref, resolved to the new id); a dangling/unknown ref on a link tile is dropped rather than failing the whole import (optional, unlike a widget tile's required one) | ✅ `api/config` |

### 3.12 Persistence / schema (`src/db/`)
| # | Case | Where |
|---|---|---|
| M1 | All `*.sql` migrations + JS layout backfill applied and recorded once | ✅ `unit/db.migrations` |
| M2 | Core tables exist (`settings,pages,integrations,tiles,migrations`) | ✅ `unit/db.migrations` |
| M3 | Reopen is idempotent (no dup page, no migration re-run) | ✅ `unit/db.migrations` |
| M4 | `tiles.type` CHECK allows all 12 types, rejects unknown | ✅ `unit/db.migrations` |
| M5 | `pages → tiles` ON DELETE CASCADE; `integrations → tiles` ON DELETE SET NULL | ✅ `unit/db.migrations` (cascade) · 🖐️ (set null) |

### 3.13 Server & static shell (`src/server.js`)
| # | Case | Where |
|---|---|---|
| E1 | `/healthz` → `{status:"ok"}` | ✅ `api/health` |
| E2 | `/` serves the SPA shell (`<div id="app"></div>`, `/app.js`) | ✅ `api/health` |
| E3 | `/app.js`, `/style.css`, `/sw.js` served; SW is network-first | ✅ `api/health` |
| E4 | Unknown `/api/*` → 404 | ✅ `api/health` |
| E5 | Boots against a fresh `DATA_DIR` with and without `APP_SECRET` | ✅ every `api/*` file (helper) |
| E6 | Graceful `SIGTERM` shutdown (poller stop, db close) | ✅ implicit via helper `stop()` |

### 3.14 Frontend (`web/`)
| # | Case | Where |
|---|---|---|
| F1 | App hydrates: `#app` populated by Preact, site title shown | ✅ `e2e/smoke` |
| F2 | Dashboard renders without a blank frame | ✅ `e2e/smoke` |
| F3 | i18n: locale switch, English fallback, `{var}` interpolation, no orphan `es` keys | ✅ `unit/web.i18n` |
| F4 | Settings forms (Appearance/Pages/Integrations/Backup/ComposeScan), tile modal, icon picker, dynamic config form | 🖐️ §9 |
| F5 | PWA install / offline shell / post-deploy auto-reload | 🖐️ §9 |
| F6 | Every built-in tile type renders its widget body | 🖐️ §9 (F1/F2 cover the shell) |

---

## 4. Adding cases when you add a feature

When you add or change a feature:

1. **Add rows to §3** under the right area (or a new subsection). Give each a
   stable id (`P12`, `T19`, …) and mark ✅/🖐️.
2. **Write the automated test** in the matching file:
   - Pure function → `test/unit/<area>.test.mjs`
   - New/changed route → `test/api/<area>.test.mjs`
   - Needs a DOM → `test/e2e/smoke.test.mjs` + a 🖐️ row in §9
3. **New shared fixture?** Put it in `test/helpers/fixtures.mjs`, not inline.
4. Run `npm test`; keep it green and under ~15s.
5. If a case genuinely can't be automated, mark it 🖐️ and add a concrete step to
   §9 — never leave a feature with no row at all.

### Case template (copy into §3)

```
| X# | <one-line behaviour, including the failure mode> | ✅ `api/<file>` / 🖐️ §9 |
```

### Test skeletons

Unit:
```js
import test from 'node:test';
import assert from 'node:assert/strict';
import { thing } from '../../src/<path>.js';

test('<area>: <behaviour> — <edge case>', () => {
  assert.equal(thing(input), expected);
});
```

API (one server per file):
```js
import { describe, it, before, after } from 'node:test';
import assert from 'node:assert/strict';
import { startServer } from '../helpers/server.mjs';

describe('<area> API', () => {
  let s;
  before(async () => { s = await startServer(null); });   // add { appSecret: 'x' } if the route needs encryption
  after(() => s.stop());

  it('<method path> — <behaviour>', async () => {
    const r = await s.request('/api/<path>', { method: 'POST', body: { ... } });
    assert.equal(r.status, 201);
    assert.equal(r.body.<field>, <expected>);
  });
});
```

---

## 5. What is deliberately NOT automated

| Area | Why | Mitigation |
|---|---|---|
| Live integration upstreams (Sonarr, Plex, qBittorrent, …) | Need real services + secrets | `unit/integrations.registry` proves every integration loads & has a valid schema; view/model helpers are unit-tested |
| open-meteo weather happy path | Third-party network in CI is flaky | 400-path tested; response shaping tested via `unit/lib.apiMap` |
| > 5 MB upload rejection, compose-walk limits, archive path-traversal | Slow / noisy to trigger; logic is small and guarded | Documented here; add a targeted test if that code is touched |
| Full visual regression of every theme/tile | Out of scope for a smoke suite | `e2e/smoke` proves the app renders; §9 manual pass for themes |

---

## 6. Environment notes

- **Node:** ≥ 22.5 (matches `package.json` engines). `node:sqlite` needs
  `--experimental-sqlite` on 22 (the npm scripts pass it; harmless on 24+).
- **No network required** for `npm test`. The feed/customapi tests spin up a
  loopback HTTP server; `host/stats` tolerates a sandbox that blocks `/proc`.
- **Browser smoke:** set `CHROME_BIN` to a Chrome/Chromium binary, or
  `npx playwright install --with-deps chromium` and it is auto-discovered. The
  suite **skips** (not fails) when no launchable browser is found.

---

## 7. Manual smoke checklist (§9)

Run after frontend changes or before a release. ~5 minutes.

1. `npm run build && npm start`, open `http://localhost:3000`.
2. **Shell:** dashboard loads, site title in the header, no console errors.
   Any scrolling surface (a scrolled widget body, the settings page, a tile
   modal) shows the slim theme-tinted scrollbar, not the browser's default —
   check on a couple of themes, including a glass/gradient one, since the
   thumb color derives from `--text-dim`.
3. **Pages:** add a page, rename it, drag tabs to reorder, delete it.
4. **Tiles:** add one of each type (link, iframe, clock, weather, notes, search,
   rss, calendar, bookmarks, customapi, resources); each renders a body. In the
   Add/Edit modal, expand "Group & appearance" on a short window (and on the
   glass/gradient themes) → the dimmed backdrop scrolls, the whole modal is
   reachable, and the Save/Cancel/Delete bar stays stuck to the bottom of the
   screen. **Clock:** outside edit mode there's no toolbar/title bar at all —
   just the time (and date, if enabled) filling the tile; entering edit mode
   brings back the drag handle and Edit button as normal. **Bookmarks:** set
   Columns to 2+ → each link row grows a "Col N" picker; assigning links to
   column 2 (say) puts them in the second column's own list regardless of add
   order, not wherever grid auto-flow would have put them; dropping Columns
   back to 1 shows one flat list again, and raising it back to the same count
   restores each link's column.
5. **Tile layout:** drag a tile to a new cell, resize it from the corner grip,
   collapse a named group, reload — layout persisted. Edit a tile and change
   its Group → it lands in the target group's first free slot (bottom), never
   on top of an existing tile; then drag it where you want.
5b. **Groups:** in a tile's "Group & appearance" section, the Group field is a
   radio list — None, every group already on the page, and "New group…"
   (name field appears only for that). Pick an existing one → tile moves under
   it on save, no typing. In page edit mode, each group heading shows ✎/✕:
   ✎ renames the group across all its tiles (collapse state follows the new
   name); ✕ asks to confirm, then its tiles become ungrouped — none deleted.
6. **Appearance:** change theme (incl. new `dracula` / `oled`), dark mode,
   accent, font — applied live.
   Tick "Custom text color", pick a colour → all page text (headings, group
   labels, compose panel, tile bodies) recolours live; secondary text follows
   a derived dimmer shade. Pick a mid-grey → a "⚠ Low contrast" hint appears.
   Untick → back to the theme. Per tile: Group & appearance → "Text color"
   recolours just that tile (same contrast hint); "Reset appearance" clears it.
   **Theme sharing:** expand it, "Export current look" fills the box (and
   copies to clipboard); paste a `{ "theme": "dracula", "accent": "#bd93f9" }`
   blob into the import box → Apply → live, bad values rejected with a message.
6b. **Per-page appearance:** Settings → Pages → a page → "Layout & appearance"
   → set Theme / Accent / Text color for that page only → switch to it and the
   look changes; other pages unaffected; blank = inherit global. Per-page JS
   textarea runs only when both its checkbox and the global "Enable custom
   JavaScript" are on. Custom CSS/JS editors are monospace and scroll sideways.
7. **Background:** paste a URL → Save → full-page image appears behind tiles.
   Upload an image → Save → same. Set per-page background + blur/dim/opacity on
   one page only; confirm it overrides the global on that page and not others.
8. **Custom CSS/JS:** add `.tile{border-radius:0}` globally; enable custom JS with
   `document.title='x'` — both take effect.
9. **Integrations:** add one (e.g. gluetun) with a bogus URL → shows an error
   status; edit the name without re-entering the password → still works.
   Add a Sonarr or Radarr integration against a real instance, then add a
   widget tile for it: the tile modal's "Show" list comes from the
   integration's type, not a global default. Add a second widget tile on the
   *same* integration with a different view checked — confirm the two tiles
   show different things (view selection is per-tile). With exactly one view
   checked and a second same-type integration configured, "Also include"
   appears; check it and confirm the tile merges both. For a **Download
   queue** view, add a qBittorrent widget tile, check only "Download queue",
   and "Also include" a SABnzbd integration → one queue with every item, each
   row labelled with its client. The "Also include" list must offer only other
   download clients here — Radarr/Sonarr (also have a `queue` view) must not
   appear, since they're in a different `mergeGroup`. Break one client's queue
   fetch temporarily (e.g. wrong password) while its other view (stats) still
   works, then trigger a poll → the merged queue keeps showing that client's
   last-good rows (not blanked) and a "stale data — <name>" banner appears atop
   the tile; fix the config and poll again → banner clears, rows update. The first "Also include"
   check (on either a widget tile or a link tile with "Include integration
   data") auto-fills a blank Title field with the view's own label ("Download
   queue") — typing a title first, or already having one, must leave it alone.
   On a **link tile** specifically, once merged, its header shows a second
   line under the title naming the merged services ("qBittorrent + Sabnzbd") —
   check that line disappears at the shortest (1×1) tile height rather than
   forcing a scrollbar, and reappears once resized taller. For "Release
   calendar" specifically, events from every source merge into one grid and a
   day cell shows only the show/movie name (no source tag). Hover (or focus) a
   day with releases → a floating card lists each release that day with its
   poster + name + episode/release tag; the card escapes the tile's clipped
   box. The calendar shows one month at a time — ‹ › page between months
   (arrows disable past the data range), a "Today" chip returns to the current
   month.
10. **Tile scaling:** drop a **weather** tile and a **calendar** widget to the
    smallest (1×1) size → text isn't clipped: weather keeps icon + temp + one
    line, the calendar grid fills the tile with day tint/dots instead of event
    titles. Resize larger → detail comes back. Also check a **stats** view
    (e.g. Radarr/Sonarr's library counts) with 4+ values at S/M size — they
    stay on one row (shrinking together, smaller type) instead of wrapping to
    a second line. Specifically on a **link tile with an attached integration**
    (icon/title header + widget body in one card) at the shortest height, a
    `stats` or `nowplaying` view fits with no vertical scrollbar — the extra
    header eats more of the row than a plain widget tile's toolbar does, so
    this is the case most likely to clip if the height-based shrink regresses.
11. **Compose panel:** with a compose dir configured, the "Ports & volumes"
    panel is one surface matching the tiles (same border/radius/shadow; frosts
    on the glass/gradient themes), full width so it lines up with the grid.
    Inside: an uppercase section-heading head row + hairline rule, a flat
    divided stack list (no nested cards), chips tinted to `--bg` for
    definition, and only published host-port chips carry a faint accent wash.
    Check it on the glass theme — the panel blurs the background like the
    tiles, not see-through.
    Type in the **filter** box → the stack list narrows on name / service /
    image / port and shows "N of M"; a non-match shows an empty-state line.
    **Expand all** / **Collapse all** toggle every stack, and the open set
    survives a reload (per page). If two services (any stack) publish the same
    `host/proto`, both chips turn red with a "clash" tag and a tooltip naming
    the other owner, and the header shows "⚠ N port conflicts".
12. **Backup:** export a zip; re-import it → app restarts, data intact.
13. **Config:** export YAML, edit the site title, re-import → applied.
14. **PWA:** install, go offline, reload → shell still loads.

---

## 8. Suite health

| Metric | Target |
|---|---|
| `npm test` runtime | < 15 s |
| Network calls in `npm test` | 0 (loopback only) |
| Flaky tests | 0 — a failing test means a real regression |
| Route coverage | 100% of `src/routes/*` endpoints |
| `src/shared` + `src/lib` function coverage | 100% of exported functions |
