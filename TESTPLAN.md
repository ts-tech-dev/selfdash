...
| I29 | **Deluge** (`mergeGroup: 'download'`): JSON-RPC `/json`; `auth.login` sets a `_session_id` cookie cached per url (concurrent first-poll logins deduped), any RPC error drops the cached cookie and retries the login once (no HTTP status to branch on — Deluge reports auth failure inside a 200 body); `queue` converts Deluge's 0-100 `progress` to the model's 0-1; `stats` counts Downloading/Seeding and sums payload rates | ✅ `unit/integrations.deluge` · `api/integrations` |
| I30 | **NZBGet** (`mergeGroup: 'download'`): JSON-RPC `/jsonrpc` with HTTP basic auth (control username/password, required by default); `queue` computes `progress` from `1 - RemainingSizeMB/FileSizeMB` per group; `stats` maps `status` (`RemainingSizeMB`, `DownloadRate` via `fmtRate`, `DownloadPaused`) plus the `listgroups` count | ✅ `unit/integrations.nzbget` · `api/integrations` |
