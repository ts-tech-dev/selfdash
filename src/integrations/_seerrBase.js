// Shared by the Overseerr and Jellyseerr integrations. Overseerr, Jellyseerr and seerr
// all speak the same API: /api/v1, X-Api-Key header. The two integration files differ
// only in their static key/title.
//
...
// { viewKey: label } for `static views`.
export const seerrViews = Object.fromEntries(Object.entries(VIEWS).map(([k, v]) => [k, v.label]));

// Same config for both — a plain URL + API key, plus an optional row cap for the list views.
export const seerrConfigFields = [
  { name: 'url', label: 'Server URL', type: 'url', required: true },
  { name: 'apiKey', label: 'API Key', type: 'password', required: true },
  { name: 'listLimit', label: 'Rows for list views (max 25)', type: 'number', required: false },
];

export function fetchSeerrData(ctx) {
